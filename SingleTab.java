	...
	public void getPrintDict(){
		...
		*/
		...
		for (int index=0; index < this.headers.get("cols").size(); index ++){
			System.out.printf("write col header {0} to x:{1} y:{2}",this.headers.get("cols").get(index), colDepth-1 , index );
		...
		for(int index=0; index < this.headers.get("rows").size(); index ++){
			System.out.printf("write row header {0} to x:{1} y:{2}",this.headers.get("rows").get(index), index , rowDepth);
