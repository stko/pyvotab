...
public class sample_table 
{
	...
	public static void main (String[] args)
	{
		...
		t6.add(new ArrayList(Arrays.asList(new String[]{"Vorname", "Nachname", "Stadt", "Straße", "Hausnummer"})));
		t6.add(new ArrayList(Arrays.asList(new String[]{"Hins", "Mueller", "Hamburg", "Postweg", "8"})));
		t6.add(new ArrayList(Arrays.asList(new String[]{"Klaus", "Meier", "Hamburg", "Feldplatz", "5"})));
		t6.add(new ArrayList(Arrays.asList(new String[]{"Klaus", "Meier", "Berlin", "Burgallee", "4"})));
		t6.add(new ArrayList(Arrays.asList(new String[]{"Klaus", "Schulze", "Berlin", "Burgallee", "3"})));
		t6.add(new ArrayList(Arrays.asList(new String[]{"Klaus", "Schulze", "Berlin", "am Deich", "9"})));
		t6.add(new ArrayList(Arrays.asList(new String[]{"Hans", "Mueller", "Berlin", "am Deich", "10"})));
		...
		for (PyvoSheet pyvot_sheet : pt.getPrintDict()){
			...
			System.out.printf("tabellen_name %s %s\n",page_name,pyvot_sheet.style.toString());
			...
			for (int row = 0 ; row < pt_table.ySize; row++){
				...
				for (int col=0; col < pt_table.xSize; col++){
                                    ...
                                    if (!pt_table.containsKey(col) || !pt_table.get(col).containsKey(row)){
					System.out.printf("<td/>\n");
                                    ...
                                    if (cell_content!=null){ // if content is None, then it"s an empty filler element, needed to handle multicell html cells correctly
                                            ...
                                            System.out.printf("%s row:%d/col:%d </td>\n",cell_content.get("value"),row,col);
                                    }
				}System.out.printf("</tr>\n");
			}
